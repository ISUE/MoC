...
public class CombineRecipe implements Comparable<CombineRecipe> {
	private String name;
	private boolean valid = true;
	
	private SpoutItemStack[] input = new SpoutItemStack[9]; 
	private SpoutItemStack[] output = new SpoutItemStack[4]; 
	private int[] outputChance = {100, 100, 100, 100};
	
	public CombineRecipe(String name, String[] input, String[] output) {
		this.name = name;
		
		for (int i = 0 ; i < 9 ; i++) {
			this.valid = parseInput(input[i], i);
			if (!this.valid) break;
			
		}
		
		for (int i = 0 ; i < 4 ; i++) {
			this.valid = parseOutput(output[i], i);
			if (!this.valid) break;
	...
	private boolean parseInput(String inputString, int i) {
		//input - id=x data=x enchant=s,s,s name=s
		if (inputString == null) {
			this.input[i] = new SpoutItemStack(0);
			return true;
		...
		if (customBlockName != null) {
			for (CustomBlock block : MaterialData.getCustomBlocks()) {
				if (block.getName().equalsIgnoreCase(customBlockName)) {
					this.input[i] = new SpoutItemStack(block, 1);
					return true;
			...
			this.input[i] = new SpoutItemStack(typeId, data);
			
			try {
				for (String ench : enchants) { if (ench != null && ench.split("-").length == 2) this.input[i].addEnchantment(Enchantment.getById(Integer.parseInt(ench.split("-")[0])), Integer.parseInt(ench.split("-")[1])); }
			} catch (NumberFormatException e) {} catch (NullPointerException e) {} catch (NullArgumentException e) {} catch (IllegalArgumentException e) {}
	...
	public int compareTo(CombineRecipe o) { return this.name.compareTo(o.name); }
